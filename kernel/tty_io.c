		...
		if (L_CANON(tty)) {
			if (c==ERASE_CHAR(tty)) {		//是否C为删除字符
				...
				if (L_ECHO(tty)) {
					...
					PUTCH(127,tty->write_q);	//127是空白
					tty->write(tty);		//这是tty_struct结构中的void(*write)函数
				}
				DEC(tty->secondary.head);		//将这个字符擦除
				continue;
			}
			if (c==STOP_CHAR(tty)) {
				tty->stopped=1;
